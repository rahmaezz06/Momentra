...
<article class="card post-card" id="post-<?= $post['id'] ?>">
  ...
  <div style="border-top:1px solid var(--border);padding:8px 12px;display:flex;gap:8px;align-items:center;">
    ...
    <input type="text" class="quick-comment-input" data-id="
    <?= $post['id'] ?>" placeholder="Add a comment…" style="flex:1;background:transparent;border:none;outline:none;color:var(--text);font-family:inherit;font-size:.85rem;min-width:0;" inputmode="text">
    <button onclick="submitComment(<?= $post['id'] ?>)" class="btn btn-ghost" 
    style="color:var(--accent);padding:4px 6px;font-weight:700;font-size:.82rem;flex-shrink:0;">Post</button>
  </div>
