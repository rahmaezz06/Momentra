...
<div class="card" style="padding:48px 24px;text-align:center;">
  ...
  <a href="<?= BASE_URL ?>/post/create" class="btn btn-primary">Share a Photo</a>
</div>
<?php else: ?>
...
<article class="card post-card" id="post-<?= $post['id'] ?>">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;">
    <a href="<?= BASE_URL ?>/profile?u=<?= h($post['username']) ?>"
    ...
    <div style="position:relative;">
      ...
      <div id="menu-<?= $post['id'] ?>" style="display:none;position:absolute;right:0;top:36px;background:var(--card);border:1px solid var(--border);border-radius:10px;min-width:140px;z-index:10;box-shadow:0 4px 20px #00000060;overflow:hidden;">
        <a href="<?= BASE_URL ?>/post/edit?id=<?= $post['id'] ?>" style="display:block;padding:10px 16px;text-decoration:none;color:var(--text);font-size:.875rem;">✏️ Edit</a>
        <button onclick="deletePost(<?= $post['id'] ?>)" style="display:block;width:100%;text-align:left;padding:10px 16px;background:none;border:none;color:#e94560;font-size:.875rem;cursor:pointer;font-family:inherit;">🗑️ Delete</button>
      </div>
    </div>
    <?php endif; ?>
  </div>
  ...
  ?>
  <a href="<?= BASE_URL ?>/post?id=<?= $post['id'] ?>" style="text-decoration:none;display:block;">
  ...
  <div style="padding:10px 12px 4px;display:flex;align-items:center;gap:2px;">
    ...
    <a href="<?= BASE_URL ?>/post?id=<?= $post['id'] ?>" class="btn btn-ghost" style="color:var(--muted);padding:8px;">
    ...
    <?php if ($post['image']): ?>
    <a href="<?= postImageUrl($post['image']) ?>" download class="btn btn-ghost" style="color:var(--muted);padding:8px;">
    ...
    <?php endif; ?>
    <button id="save-btn-<?= $post['id'] ?>" onclick="toggleSave(<?= $post['id'] ?>)" style="background:none;border:none;cursor:pointer;padding:8px;color:<?= $post['user_saved'] ? 'var(--accent)' : 'var(--muted)' ?>;display:flex;align-items:center;margin-left:auto;">
      <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="<?= $post['user_saved'] ? 'currentColor' : 'none' ?>" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16l-7-3.5L5 21V5z"/></svg>
    </button>
  </div>
  ...
  <div style="padding:0 14px 12px;">
    ...
    <p style="font-size:.875rem;line-height:1.5;"><strong><?= h($post['username']) ?></strong> <?= nl2br(h($post['caption'])) ?></p>
    <?php endif; ?>
    <?php if ($post['comment_count'] > 0): ?>
    <a href="<?= BASE_URL ?>/post?id=<?= $post['id'] ?>" style="color:var(--muted);font-size:.82rem;text-decoration:none;display:block;margin-top:4px;">View all <?= $post['comment_count'] ?> comments</a>
    <?php endif; ?>
    <div style="color:var(--muted);font-size:.7rem;margin-top:5px;text-transform:uppercase;letter-spacing:.05em;"><?= timeAgo($post['created_at']) ?></div>
  </div>
  ...
  <div style="border-top:1px solid var(--border);padding:8px 12px;display:flex;gap:8px;align-items:center;">
    ...
    <input type="text" class="quick-comment-input" data-id="<?= $post['id'] ?>" placeholder="Add a comment…" style="flex:1;background:transparent;border:none;outline:none;color:var(--text);font-family:inherit;font-size:.85rem;min-width:0;" onkeydown="if(event.key==='Enter')submitComment(<?= $post['id'] ?>)">
