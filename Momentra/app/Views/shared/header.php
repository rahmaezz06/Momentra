...
body{font-family:'DM Sans',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;padding-bottom:80px;transition:background-color .3s,color .2s;}
nav{position:fixed;top:0;left:0;right:0;z-index:100;height:var(--nav-h);background:var(--surface);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;padding:0 16px;gap:8px;}
.nav-logo{font-family:'Playfair Display',serif;font-size:1.4rem;background:var(--gradient);-webkit-background-clip:text;-webkit-text-fill-color:transparent;text-decoration:none;flex-shrink:0;}
.nav-links{display:flex;align-items:center;gap:4px;flex-shrink:0;}
.nav-links a{display:flex;align-items:center;gap:6px;color:var(--muted);text-decoration:none;padding:8px 10px;border-radius:10px;font-size:.875rem;font-weight:500;transition:all .2s;white-space:nowrap;}
...
.nav-search input{width:100%;background:var(--hover-bg);border:1.5px solid var(--border);border-radius:22px;padding:8px 14px 8px 38px;color:var(--text);font-size:.85rem;outline:none;transition:border .2s,box-shadow .2s;}
.nav-search input:focus{border-color:var(--accent);box-shadow:0 0 0 3px #6366f120;}
.nav-search .search-icon{position:absolute;left:11px;top:50%;transform:translateY(-50%);color:var(--muted);width:16px;height:16px;pointer-events:none;}
#search-results{position:absolute;top:46px;left:0;right:0;background:var(--card);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;display:none;z-index:200;box-shadow:0 8px 32px #00000050;max-height:320px;overflow-y:auto;}
...
.btn{display:inline-flex;align-items:center;gap:6px;padding:9px 18px;border-radius:10px;font-family:inherit;font-size:.875rem;font-weight:600;cursor:pointer;border:none;text-decoration:none;transition:all .2s;white-space:nowrap;}
...
.form-group input,.form-group textarea,.form-group select{width:100%;background:var(--surface);border:1.5px solid var(--border);border-radius:10px;padding:11px 14px;color:var(--text);font-family:inherit;font-size:.9rem;outline:none;resize:vertical;transition:border .2s,box-shadow .2s;-webkit-appearance:none;}
...
.bottom-nav{display:none;position:fixed;bottom:0;left:0;right:0;background:var(--surface);border-top:1px solid var(--border);padding:6px 0 max(6px,env(safe-area-inset-bottom));z-index:100;justify-content:space-around;align-items:center;}
.bottom-nav a{display:flex;flex-direction:column;align-items:center;gap:2px;color:var(--muted);text-decoration:none;font-size:.6rem;font-weight:500;padding:4px 16px;border-radius:10px;transition:color .2s;min-width:60px;}
.bottom-nav a svg{width:24px;height:24px;}
.bottom-nav a.active,.bottom-nav a:hover{color:var(--accent);}
.theme-toggle{position:fixed;bottom:72px;right:16px;z-index:99;background:var(--card);border:1px solid var(--border);border-radius:40px;padding:6px;display:flex;gap:2px;box-shadow:0 4px 16px #0000003a;}
.theme-btn{background:transparent;border:none;border-radius:32px;padding:6px 10px;cursor:pointer;font-size:1rem;transition:all .2s;color:var(--muted);}
...
@media(max-width:640px){
  ...
  .nav-links.auth-links{display:flex;}
...
<nav>
  <a href="<?= BASE_URL ?>/" class="nav-logo">Momentra</a>
  ...
  <div class="nav-links<?= !$me ? ' auth-links' : '' ?>">
    <?php if ($me): ?>
      ...
      <a href="<?= BASE_URL ?>/post/create">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
        New Post
      </a>
      <a href="<?= BASE_URL ?>/profile?u=<?= h($me['username']) ?>">
  ...
  searchTimer = setTimeout(async () => {
    try {
      ...
      searchResults.innerHTML = users.map(u => `
        <a href="<?= BASE_URL ?>/profile?u=${encodeURIComponent(u.username)}">
          <img src="${u.avatar_url || '<?= BASE_URL ?>/img/default-avatar.png'}" width="36" height="36" alt="">
          <div>
...
(function(){
  ...
  function buildToggle() {
    ...
    [['light','☀️'],['dark','🌙']].forEach(([t,icon]) => {
      ...
      btn.dataset.theme = t; btn.textContent = icon;
  ...
  applyTheme(saved);
  document.readyState === 'loading' ? document.addEventListener('DOMContentLoaded', buildToggle) : buildToggle();
})();
</script>
<?php endif; ?>
