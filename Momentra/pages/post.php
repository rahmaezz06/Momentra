...
async function refreshCsrf() {
  try {
    const r = await fetch(BASE + '/api/csrf_token.php');
    const d = await r.json();
    if (d.csrf_token) CSRF = d.csrf_token;
  } catch(e) {}
}

async function fetchWithCsrf(url, bodyData) {
  const doFetch = () => fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: bodyData + '&csrf_token=' + encodeURIComponent(CSRF)
  });
...
async function submitComment(parentId = 0) {
  ...
  try {
    let bodyData = `post_id=${postId}&body=${encodeURIComponent(body)}`;
    if (parentId) bodyData += `&parent_id=${parentId}`;

    const data = await fetchWithCsrf(BASE + '/api/comment.php', bodyData);
    ...
    if (data.success) {
      if (parentId) {
        ...
        if (repliesContainer) {
          repliesContainer.insertAdjacentHTML('beforeend', createReplyHtml(data.comment));
          const replyBtn = document.querySelector(`.reply-btn[data-id="${parentId}"] .reply-count`);
          if (replyBtn) replyBtn.textContent = (parseInt(replyBtn.textContent) || 0) + 1;
        }
      } else {
      ...
      alert(data.message || data.error || 'Could not post comment. Please try again.');
